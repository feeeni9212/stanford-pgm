...
for i = 1:numGenotypes
    p1AllelePair = genotypesToAlleles(i,:);
    for j = 1:numGenotypes
        p2AllelePair = genotypesToAlleles(j,:);
        for a1 = p1AllelePair
            for a2 = p2AllelePair
                childGenotype = allelesToGenotypes(a1,a2);
                genotypeFactor = SetValueOfAssignment(genotypeFactor,...
                    [childGenotype i j], GetValueOfAssignment(genotypeFactor, [childGenotype i j])+0.25);
            end
        end
    end
end
