...
genotypeFactor.card = [numGenotypes];

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
for i = 1:numGenotypes
    allelePair = genotypesToAlleles(i,:);
    genotypeFactor.val(i) = alleleFreqs(allelePair(1))*alleleFreqs(allelePair(2));
    if allelePair(1) ~= allelePair(2)
        % Double the probability if the two alleles are the same.
