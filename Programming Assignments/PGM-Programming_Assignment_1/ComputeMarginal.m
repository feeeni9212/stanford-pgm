...
J = ComputeJointDistribution(F);        % compute joint distribution
JObs = ObserveEvidence(J, E);           % incorporate evidence
JObs.val = JObs.val / sum(JObs.val);    % normalize the factor
M = FactorMarginalization(JObs, setdiff(JObs.var, V));
