...
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
numClusters = length(F);
numSingletons = length(unique([F.var]));
P.clusterList = F;
P.edges = zeros(numClusters, numClusters);

for i = 1:numSingletons
    for j = numSingletons+1:numClusters
        if ismember(P.clusterList(i).var, P.clusterList(j).var)
            P.edges(i, j) = 1;
            P.edges(j, i) = 1;
        end
    end
