...
for m = 1:length(edgeFromIndx),
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [MESSAGES(i, j).var, indx, ~] = intersect(P.clusterList(i).var, P.clusterList(j).var);
    MESSAGES(i, j).card = P.clusterList(i).card(indx);
...
while (iteration < 100000),
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    newMessage = P.clusterList(i);
    for k = setdiff(find(P.edges(i, :)), j)
        newMessage = FactorProduct(newMessage, MESSAGES(k, i));
    end
    % marginalize
    varsToSumOut = setdiff(newMessage.var, P.clusterList(j).var);
    newMessage = FactorMarginalization(newMessage, varsToSumOut);
    % normalize
    newMessage.val = newMessage.val / sum(newMessage.val);
    
    MESSAGES(i, j) = newMessage;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
%     % store the differences between messages
%     if i == 19 && j == 3
%         r19to3 = [r19to3; MessageDelta(prevMessage, newMessage)];
%     elseif i == 15 && j == 40
%         r15to40 = [r15to40; MessageDelta(prevMessage, newMessage)];
%     elseif i == 17 && j == 2
%         r17to2 = [r17to2; MessageDelta(prevMessage, newMessage)];
%     end
...
% figure();
% plot([r19to3 r15to40 r17to2]);
% currAxis = axis;
% currAxis(4) = 0.0001;
% axis(currAxis);
% legend('residuals of message: 19 to 3', 'residuals of message: 15 to 40', ...
%     'residuals of message: 17 to 2');
