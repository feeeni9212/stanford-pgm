...
N = length(P.clusterList);              % number of clusters
numMessages = length(find(P.edges));    % number of possible messages
m = mod(m, numMessages);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for j = 1:N
    neighbors = find(P.edges(:, j));
    if m >= length(neighbors)
        m = m - length(neighbors);
    else
        i = neighbors(m+1);
        break;
    end
